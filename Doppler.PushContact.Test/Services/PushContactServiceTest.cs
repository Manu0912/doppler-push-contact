...
with {nameof(deviceToken)} {deviceToken}. {PushContactDocumentProps.EmailPropName} can not be updated with following value: {email}"),
        ...
        [Fact]
        public async Task HasPushNotificationEnabledByVisitorGuid_should_throw_exception_and_log_error_when_push_contacts_cannot_be_getter_from_storage()
        {
            // Arrange
            var fixture = new Fixture();
            ...
            // Act
            // Assert
            await Assert.ThrowsAsync<Exception>(() => sut.HasPushNotificationEnabledByVisitorGuid(visitorGuid));
        ...
        [Fact]
        public async Task HasPushNotificationEnabledByVisitorGuid_should_return_a_boolean()
        {
            // Arrange
            List<BsonDocument> allPushContactDocuments = FakePushContactDocuments(10);
            ...
            // Act
            var result = await sut.HasPushNotificationEnabledByVisitorGuid(visitorGuidFilter);
